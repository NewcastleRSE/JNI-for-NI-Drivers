...
public class NiDaq {
    ...
    public void createAIVoltageChannel(Pointer taskHandle, String physicalChannel, String nameToAssignToChannel,
                                       int terminalConfig, double minVal, double maxVal, int units,
                                       String customScaleName) throws NiDaqException {
        ...
        checkError(Nicaiu.INSTANCE.DAQmxCreateAIVoltageChan(taskHandle, btPhysicalChannel, btNameOfChannel,
                terminalConfig, minVal, maxVal, units, btCustomScaleName));
    }
     ...
     * @throws NiDaqException
     */
    public void createAICurrentChannel(Pointer taskHandle, String physicalChannel, String nameToAssignToChannel,
                                       int terminalConfig, double minVal, double maxVal, int units, int shuntResistorLoc,
                                       double extShuntResistorVal, String customScaleName) throws NiDaqException {
        byte[] btPhysicalChannel = (physicalChannel + " ").getBytes();
        btPhysicalChannel[btPhysicalChannel.length - 1] = 0;
    ...
    public void writeDigitalLines(Pointer taskHandle, int numSamplesPerChannel, int autoStart, double timeOut,
                                  int dataLayout, byte[] data) throws NiDaqException {
     ...
     * @throws NiDaqException
     */
    public void createAOVoltageChan (Pointer taskHandle, byte physicalChannel[], byte nameToAssignToChannel[],
                                    double minVal, double maxVal, int units, byte customScaleName[]) throws NiDaqException {
        byte[] btPhysicalChannel = (physicalChannel + " ").getBytes();
        btPhysicalChannel[btPhysicalChannel.length - 1] = 0;
        byte[] btNameOfChannel = (nameToAssignToChannel + " ").getBytes();
        btNameOfChannel[btNameOfChannel.length - 1] = 0;
        ...
        checkError(Nicaiu.INSTANCE.DAQmxCreateAOVoltageChan(taskHandle, btPhysicalChannel, btNameOfChannel, minVal,
                maxVal, units, btCustomScaleName));

    }

    public void DAQmxWriteAnalogF64(Pointer taskHandle, int numSampsPerChan, NativeLong autoStart, double timeout,
                         NativeLong dataLayout, DoubleBuffer data, IntByReference sampsPerChanWritten, NativeLong reserved)
            throws NiDaqException {


        checkError(Nicaiu.INSTANCE.DAQmxWriteAnalogF64(taskHandle, numSampsPerChan, autoStart, timeout,
                dataLayout, data, null, null));
    }
